...
class Admin extends BaseBussiness
{
    public function doLogin($reqData){
        ...
        if($admin['status'] != 1){
            $this->result['status'] = -1;
            $this->result['msg'] = '该账户已被禁用，请联超级管理员';
            return $this->result;
        }
    ...
    public function updateAdmin($id,$reqData){
        //判断该账户是否存在
        $isset = $this->getModel('Admin')->getById($id,'id');
        if(!$isset){
            $this->result['status'] = -1;
        ...
        $adminName = $this->getModel('Admin')->getByAdmin($reqData['admin_name'],'id');
        if($adminName && $id != $adminName['id']){
            $this->result['status'] = -1;
            $this->result['msg'] = '该账户名已被使用';
            return $this->result;
        }
        ...
        //判断手机号是否重复
//        $adminFromSql = $this->getModel('Admin')->getPhone($reqData['phone'],'id');
//        if($adminFromSql){
//            $this->result['status'] = -1;
//            $this->result['msg'] = '该手机号已被使用';
//            return $this->result;
//        }
        ...
        //修改账户信息
        unset($reqData['id']);
        $update = $this->getModel('Admin')->updateById($id,$reqData);
        if(!$update){
            $this->result['status'] = -1;
            $this->result['msg'] = '修改失败';
            return $this->result;
