...
//        exit;
    ...
    public function loginQueryAction(){
        $start_time = $this->request->getQuery('start_time');
        $end_time = $this->request->getQuery('end_time');
        $type = $this->request->getQuery('type');
        ...
        //查询数据
        if($type == 'day'){
            $sql = "select count(id) as `count`,`date` from homepage_login_log where `time` >= $start_time and `time` <= $end_time group by `date`;";
            $result=$this->db->query($sql);
            $result->setFetchMode(\Phalcon\Db::FETCH_ASSOC);
            $result = $result->fetchAll();
        }elseif($type == 'month'){
            $sql = "select count(id) as `count`,`month` as `date` from homepage_login_log where `time` >= $start_time and `time` <= $end_time group by `month`;";
            $result=$this->db->query($sql);
            $result->setFetchMode(\Phalcon\Db::FETCH_ASSOC);
            $result = $result->fetchAll();
        }
