...
//        $this->add('/api/transfer','transfer::transfeStation');         //中转站(回调地址)
        ...
        $this->add('/manager/prop/crontab','send::propCrontabView');        //定时20时发送道具（页面）
        $this->addPost('/manager/prop/crontab','send::propCrontab');        //定时20时发送道具
        $this->add('/manager/propCrontable/deal','send::propCrontabDeal');  //定时20时发送道具（处理）
        $this->add('/manager/prop/plan','send::propPlanView');              //定时发送道具（页面）
        $this->addPost('/manager/prop/plan','send::propPlan');              //定时发送道具
        $this->add('/manager/prop/planDeal','send::planDeal');              //定时发送道具（处理）
